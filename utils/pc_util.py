...
### Covert point to volumetric field
def trilinear_interpolation_old(points, vs_x=0.1, vs_y=0.1, vs_z=0.1, xmin=-3.84, xmax=3.84, ymin=-3.84, ymax=3.84, zmin=-0.2, zmax=2.68):
    '''
...
def trilinear_interpolation(points, vs_x=0.1, vs_y=0.1, vs_z=0.1, xmin=-3.84, xmax=3.84, ymin=-3.84, ymax=3.84, zmin=-0.2, zmax=2.68):
    ...
    vy = int((ymax-ymin)/vs_y)
    vz = int((zmax-zmin)/vs_z)
    vox = torch.zeros((points.shape[0], vx,vy,vz)).float().cuda()
