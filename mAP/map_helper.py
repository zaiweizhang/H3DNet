...
def parse_predictions(end_points, config_dict):
    ...
    @Args:
        end_points: dict, {
            pred_bbox (B, num_proposal, 8) = center, size, angle, semantic
            objectness_scores (B, num_proposal) 
            sem_cls_probs (B, num_proposal, num_class) 
            nonempty_box_mask (B, num_proposal)

            gt_bbox(B, num_max_gt, 8), center, size, angle, semantic
            gt_bbox_mask(B, num_max_gt)}
        config_dict: dict
...
def parse_groundtruths(end_points, config_dict):
    ...
    """
    center_label = end_points['gt_bbox'][:, :, :3]
    sem_cls_label = end_points['gt_bbox'][:, :, -1]
    box_label_mask = end_points['gt_bbox_mask']
