...
def parse_predictions(end_points, config_dict):
    ...
    for i in range(bsize):
        for j in range(num_proposal):
            heading_angle = config_dict['dataset_config'].class2angle2(\
...
def parse_groundtruths(end_points, config_dict):
    ...
    for i in range(bsize):
        for j in range(K2):
            ...
            heading_angle = config_dict['dataset_config'].class2angle2(heading_class_label[i,j].detach().cpu().numpy(), heading_residual_label[i,j].detach().cpu().numpy())
