...
def compute_sem_cls_loss(end_points, config, addp=False):
    """ Compute 3D bounding box and semantic classification loss.
    ...
    sem_cls_loss = criterion_sem_cls(end_points['pred_sem_class'], end_points['sem_cls_label'])

    if addp:
        criterion_sem_cls = nn.BCEWithLogitsLoss()
        support_cls_loss = criterion_sem_cls(end_points['pred_support_class_pred'].transpose(2,1), end_points['support_cls_label'])
        return sem_cls_loss+support_cls_loss, support_cls_loss
    else:
        return sem_cls_loss,0# support_cls_loss

def compute_support_cls_loss(end_points, config):
    """ Compute 3D bounding box and semantic classification loss.
    ...
    criterion_sem_cls = nn.BCEWithLogitsLoss()
    support_cls_loss = criterion_sem_cls(end_points['pred_support_class'].transpose(2,1), end_points['support_cls_label'])

    return support_cls_loss

def get_loss(end_points, config, addp=False):
    """ Loss functions
    ...
    loss1, loss3 = compute_sem_cls_loss(end_points, config, addp=addp)
    loss2 = 0#compute_support_cls_loss(end_points, config)
    ...
    loss = loss1+loss2#sem_cls_loss
    #loss *= 10
    end_points['loss'] = loss
    end_points['loss1'] = loss1
    #end_points['loss2'] = loss2
    #end_points['loss3'] = loss3
