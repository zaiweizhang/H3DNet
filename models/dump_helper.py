...
def dump_results(end_points, dump_dir, config, dataset, mode='center'):
    '''
    ...
    pred_heading_residual = torch.gather(end_points['heading_residuals'+mode], 2, pred_heading_class.unsqueeze(-1)) # B,num_proposal,1
    ...
    pred_mask = end_points['pred_mask'+mode] # B,num_proposal
    ...
    for i in range(batch_size):
        ...
        print(scan)
        print('-' * 30)
        #pcd_pc = myutils.create_pointcloud_from_points(point_clouds[i,:,:3], [0.9, 0.9, 0.9])
        #pcd_pred_center = myutils.create_pointcloud_from_points(pred_center[i], [0, 1, 0])
        #pcd_gt_center = myutils.create_pointcloud_from_points(gt_center[i], [1, 0, 0])
        #lineset_pred_box_list = []
        #lineset_gt_box_list = []
        box_pred_list = []
        box_gt_list = []
        obb_pred_list = []
        ...
        # box_pred_nms_list = [box_pred_list[i] for i in np.logical_and(objectness_prob>DUMP_CONF_THRESH, pred_mask[i,:]==1)]
...
def dump_objcue(input_points, end_points, dump_dir, config, dataset, inference_switch=False):
    ...
    for i in range(len(seed_gt_center)):
        ...
        sio.savemat(os.path.join(dump_dir,dataset.scan_names[end_points['scan_idx'][i]]+'_point_objcue.mat'), {'full_pc': point_clouds[i,:,:3], 'sub_pc':seed_xyz[i,...], 'subpc_mask':seed_gt_mask[i,...], 'gt_center': seed_gt_center[i,...], 'gt_corner': seed_gt_corner[i,...], 'pred_center': seed_pred_center[i,...], 'pred_corner': seed_pred_corner[i,...]})
