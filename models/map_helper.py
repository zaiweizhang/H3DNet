...
def parse_predictions(end_points, config_dict):
    ...
    @Args:
        end_points: dict, {
            pred_bbox(B, 256, 8), center, size, angle, semantic
            obj_prob(B, 256)
            bboxes_gt(B, 64, 8), center, size, angle, semantic
            bboxes_gt_mask(B, 64)}
        config_dict: dict
    ...
    """
    pred_center = end_points['pred_bbox'][:, :, :3] # B,num_proposal,3
    pred_sem_cls = end_points['pred_bbox'][:, :, 7] # B,num_proposal
    ...
    if config_dict['remove_empty_box']:
        pass
        # print('Votenet did not use it, nor do we! ')
        # -------------------------------------
    ...
    # obj_logits = end_points['objectness_scores'].permute(0, 2, 1).detach().cpu().numpy() # (B, 2, 256) -> (B, 256, 2) 
    # obj_prob = softmax(obj_logits)[:,:,1] # (B,K), here!!!
    obj_prob = end_points['obj_prob'].detach().cpu().numpy()
    ...
    for i in range(bsize):
        if config_dict['per_class_proposal']:
            ...
            for ii in range(config_dict['dataset_config'].num_class):
                cur_list += [(ii, pred_corners_3d_upright_camera[i,j], obj_prob[i,j]) \
                    for j in range(pred_center.shape[1]) if pred_mask[i,j]==1 and obj_prob[i,j]>config_dict['conf_thresh']] # here, why conf_thresh is only 0.05?
...
def parse_groundtruths(end_points, config_dict):
    ...
    @Returns:
        batch_pred_map_cls: a list of len == batch size (BS)
            ...
            where j = 0, ..., num of valid detections - 1 from sample input i
    """
    center_label = end_points['bboxes_gt'][:, :, :3]
    sem_cls_label = end_points['bboxes_gt'][:, :, 7]
    box_label_mask = end_points['bboxes_gt_mask']
    ...
    for i in range(bsize):
        for j in range(K2):
            ...
            heading_angle = end_points['bboxes_gt'][i, j, 6].detach().cpu().numpy()
            box_size = end_points['bboxes_gt'][i, j, 3:6].detach().cpu().numpy()
