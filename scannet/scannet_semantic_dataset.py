...
class ScannetSemanticDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        isvalid = False
        for i in range(10):
            curcenter = point_cloud[np.random.choice(len(semantic_labels),1)[0],:]
            ...
            cur_support_labels = support_labels[curchoice]
        ...
        support_labels = cur_support_labels[choice]
	#mask = mask[choice]
	...
	#sample_weight *= mask
        ...
        ret_dict = {}
        ret_dict['point_clouds'] = point_cloud.astype(np.float32)
        ret_dict['sem_cls_label'] = semantic_labels.astype(np.int64)
        ret_dict['support_cls_label'] = support_labels.astype(np.float32)
        return ret_dict
